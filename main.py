...
def calibrate_magnetic_wifi_ibeacon_to_position(path_file_list):
    ...
    for path_filename in path_file_list:
        ...
        if wifi_datas.size != 0:
            ...
            # split wifi lists w.r.t. unique timestamps
            wifi_datas_list = split_ts_seq(wifi_datas, sep_tss)

            for wifi_ds in wifi_datas_list:
                # calculate difference between step positions timestamps
                # and wifi timestamps
                diff = np.abs(step_positions[:, 0] - float(wifi_ds[0, 0]))
                # find the closest corresponding position index to the
                # wifi message
...
def extract_wifi_rssi(mwi_datas):
    ...
    for position_key in mwi_datas:
        ...
        for wifi_d in wifi_data:
            ...
            # store the average rssi and count of wifi according to its (bssid, position)
...
if __name__ == "__main__":
    ...
    print(f"There are a total of {len(path_filenames)} to process")
    print("Visualizing ground truth positions w.r.t dead reckoning...")
    col_names = ["TIMESTAMP"] + flatten_dict(SENSOR_FIELDS)
    dx_names = ["filename", "lineno"]
    df = pd.DataFrame(index=idx_names, columns=col_names)
    for i, path_filename in enumerate(path_filenames[:5]):
        ...
        acce_datas = get_sensor_values(
            sensor_data, [TYPE_ACCELEROMETER], with_ts=True, to_val=True, dropna=True
        )
        ahrs_datas = get_sensor_values(
            sensor_data, [TYPE_ROTATION_VECTOR], with_ts=True, to_val=True, dropna=True
        )

        step_positions = compute_step_positions(acce_datas, ahrs_datas, waypoints)

        # fig = visualize_trajectory(waypoints[:, 1:], floor_plan_filename, width_meter, height_meter, title=path_id, show=False)
        # visualize_trajectory(step_positions[:, 1:], floor_plan_filename, width_meter, height_meter, title=path_id, show=True, fig=fig)
        ...
        sensor_data = sensor_data.reindex(columns=df.columns)
        index = pd.MultiIndex.from_product([[path_filename], sensor_data.index])
        df.loc[index] = sensor_data
